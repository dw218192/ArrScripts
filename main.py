...
class RadarrSonarrMonitor(BaseMonitor):
    ...
    class RecordScope:
        def _sync_record(self):
            ...
            for movie in self.record.movies:
                if self.queue.get_record_from_id(movie.id) is None:
                    self.record.movies.remove(movie)

        def __init__(
            self, monitor: "RadarrSonarrMonitor", queue: QueueResourcePagingResource
        ):
            ...
            if self.record_path.exists():
                ...
                self.record = RadarrSonarrMonitor.Record(lastRun=str(TimeStamp.now()))
            self.tasks = []
        ...
        async def __aexit__(self, exc_t, exc_v, exc_tb):
            ...
            self.record_path.write_text(
                json.dumps(asdict(self.record)), encoding="utf-8"
            )
    ...
    async def _run(self, cur_iter: int):
        self.logger.info(f"===== Running monitor iteration {cur_iter} =====")
        media_list = await self._list_media()

        async with self.RecordScope(self, media_list) as record_scope:
